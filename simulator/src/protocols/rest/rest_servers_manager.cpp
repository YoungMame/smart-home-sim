#include "rest_servers_manager.hpp"

#include <stdexcept>

RestServersManager::RestServersManager(int start_port)
    : start_port_(start_port), next_port_(start_port_) {
    if (start_port <= 0 || start_port > 65535) {
        throw std::invalid_argument("REST start port must be in range 1..65535");
    }
}

std::string RestServersManager::assign_endpoint(const std::string& device_id,
    ...
    if (next_port_ > 65535) {
        throw std::runtime_error("No available REST ports left for assignment");
    }
...
std::optional<std::string> RestServersManager::endpoint_for(const std::string& device_id) const {
    std::scoped_lock lock(mutex_);
...
bool RestServersManager::has_device(const std::string& device_id) const {
    std::scoped_lock lock(mutex_);
    return endpoints_by_device_id_.find(device_id) != endpoints_by_device_id_.end();
}

void RestServersManager::remove_device(const std::string& device_id) {
    std::scoped_lock lock(mutex_);
    endpoints_by_device_id_.erase(device_id);
}

void RestServersManager::clear() {
    std::scoped_lock lock(mutex_);
    endpoints_by_device_id_.clear();
    next_port_ = start_port_;
}

std::string RestServersManager::normalize_base(const std::string& base_endpoint) {
